...
- Agent health coverage includes meter capture failure/recovery, device unavailable/xrun, clipping, flatline, configurable low-signal, first-pass channel correlation with deterministic quality evidence and fault scores, disk/CPU pressure, audio-backend unavailable/recovery details, capture start/runtime/too-small/growth failure with structured stall/render evidence, cache upload failure with upload/output evidence, recorder-cache cleanup/delete-failure/tracking sync and tracking failure, recording-job claim-next/status-poll/control-plane/channel-map failure, and controller-terminal job state handoff.
