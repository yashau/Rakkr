...
- Agent health coverage includes meter capture failure/recovery, device unavailable/xrun, clipping, flatline, configurable low-signal, first-pass channel correlation with deterministic quality evidence and fault scores, disk/CPU pressure, controller Date clock-skew warning/recovery, audio-backend unavailable/recovery details, capture start/runtime/too-small/growth failure with structured stall/render evidence, capture runtime restart with recovered segment stitching, upload-pending retry and uploaded-finalization restart recovery, power-loss tiny partial rejection, controller lease expiry recovery for orphaned running jobs, disk preflight and in-flight disk cleanup recovery, stable ALSA capture-interface renumbering refresh, cache upload failure with upload/output evidence, channel-map application sync, recorder-cache cleanup/delete-failure/tracking sync and tracking failure, recording-job claim-next/status-poll/control-plane/channel-map failure, and controller-terminal job state handoff.
...
- Fake-controller smoke coverage exercises controller-synced generic meter capture-failed, xrun, device-unavailable, and recovery health with synthetic fallback without audio hardware.
- Agent system-health coverage exercises disk/CPU pressure and recovery plus controller Date clock-skew warning/recovery and audio-backend unavailable/recovery detail transitions, and fake-controller smoke exercises controller-synced audio-backend unavailable/recovery health with refreshed inventory probes, stalled-capture structured growth evidence and render-failure command/output evidence without audio hardware.
- Fake-controller smoke coverage exercises controller-synced node-heartbeat, meter-frame, listen-monitor chunk metadata, and node-config polling failure/recovery evidence without audio hardware.
...
- HDA Intel PCH flatline health smoke validates the daemon meter loop writes a local `agent.meter.flatline` health event from real silent ALSA hardware without synthetic fallback; the health sync smoke also validates the same event is posted to a fake controller with heartbeat and meter-frame sync.
...
| X32 low-signal health smoke and X32 low-signal health sync smoke | `scripts/agent-alsa-health-smoke.sh` |
| HDA Intel PCH S16_LE hardware meter smoke | `scripts/agent-alsa-meter-smoke.sh` |
| HDA Intel PCH flatline health smoke and HDA Intel PCH flatline health sync smoke | `scripts/agent-alsa-health-smoke.sh` |
...
| Agent meter quality, health-event quality evidence and fault scores, speech/noise/broadband-noise/SNR/intelligibility/hum/static/channel correlation, clipping, synthetic PCM calibration | `crates/recorder-agent/src/telemetry.rs`, `crates/recorder-agent/src/main/tests.rs` |
| Agent clipping, flatline, low-signal, channel correlation, xrun, node-heartbeat/node-config sync, clock-skew, and system-health/audio-backend transitions | `crates/recorder-agent/src/main.rs`, `crates/recorder-agent/src/agent_recovery.rs`, and `crates/recorder-agent/src/system_health.rs` |
| Agent capture start/runtime/too-small/growth, cache upload, channel-map application sync, recorder-cache cleanup/delete-failure/tracking sync and tracking failure, job claim-next/status-poll/control-plane/channel-map failure, controller-terminal handoff, upload-boundary restart recovery, controller lease expiry recovery, disk preflight and in-flight disk cleanup recovery, stable ALSA capture-interface renumbering refresh, render-failure, refreshed inventory audio-backend recovery, and system-health smoke | `apps/api/test/recording-job-lease-runner.test.ts`, `scripts/agent-fake-controller-smoke.mjs`, `scripts/agent-fake-controller-smoke-recovery.mjs`, `scripts/agent-fake-controller-smoke-support.mjs`, `crates/recorder-agent/src/inventory.rs`, `crates/recorder-agent/src/recording_job_disk.rs`, `crates/recorder-agent/src/recording_job_recovery.rs`, `crates/recorder-agent/src/recording_job_segments.rs`, `crates/recorder-agent/src/recording_job_upload.rs`, `crates/recorder-agent/src/recorder_cache_retention.rs` |
