...
| Recorder agent | 🟨 | Inventory, S16/S32 meters, controller capacity polling, bounded concurrent jobs, capture start/runtime/too-small/growth guards with structured stall/render evidence, profile rendering, controller-terminal job handoff, recorder-cache cleanup/delete-failure/tracking sync and tracking failure, clipping/flatline/low-signal/channel-correlation/heartbeat/node-config/claim-next/status-poll/control-plane/channel-map health, concurrent-safe health log, hardware-derived health-event controller sync |
...
| Health watchdog | 🟨 | Checked scheduled and agent low-signal, speech/noise, SNR, intelligibility, hum/static/broadband/correlation telemetry, deterministic agent meter-health quality evidence/fault scores, policy-tuned broadband quality alerts, synthetic calibration, field calibration helper, offline, local-log, metrics, timeline, central health workbench, node health lifecycle controls, audio-backend unavailable/recovery details, node heartbeat/config sync failure/recovery, recording-job capture start/runtime/too-small/stall/render evidence, recorder-cache cleanup/delete-failure/tracking sync, claim-next/status-poll/control-plane/channel-map failure and controller-terminal handoff, X32 hardware low-signal local/synced health, and PCH hardware flatline local/synced health; long-duration real-room validation remains |
...
344. ✅ Add fake-controller smoke coverage for agent node-config, node-heartbeat, recording-job capture start/runtime/too-small/stall/render evidence, recorder-cache cleanup/tracking sync and tracking failure, claim-next/status-poll/control-plane/channel-map failure/recovery health, controller-terminal handoff, deterministic agent meter-health fault scores, and recorder-cache delete-failure helper coverage.
## Open Questions
