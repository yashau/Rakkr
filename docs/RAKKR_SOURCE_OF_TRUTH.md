...
- Debian test rig full-agent loopback job smoke validates a fake-controller claim, ALSA `arecord` capture from `snd-aloop`, job heartbeat/status polling, WAV cache upload, recorder-cache cleanup health, and non-silent uploaded speech metadata.
...
342. ✅ Add Debian ALSA loopback full-agent job capture/upload smoke.
