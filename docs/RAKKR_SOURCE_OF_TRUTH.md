...
Rooms are a first-class entity (stable `roomId`, unique per site+name). Room ownership is per-channel: each of a recorder node's audio channels is assigned to a room, so a single node's channels can feed different rooms and two rooms can record from one node at once on their own channels (a channel with no room inherits the node default). Schedules and recordings carry their own room. The console has a Rooms page listing all rooms and a room detail view with editable name/location/notes, the room's channel/node inventory, upcoming scheduled occurrences (with who booked them), and the room's recent recordings. Access to a room is granted per-room via a roster of users/groups with independently-toggled per-action capabilities (see RBAC), organized so the average operator gets room-scoped listen/operate/view rights without any node or settings management. Existing free-text node/schedule locations backfill into rooms on migration. External calendar ingestion (Microsoft 365 room mailboxes → Rakkr schedules) is the planned next phase.
