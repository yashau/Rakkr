...
- Debian test rig X32 full-agent job smoke validates short and longer fake-controller claims, ALSA `arecord` capture from `hw:CARD=XUSB,DEV=0`, job heartbeat/status polling, 32-channel `S32_LE` WAV cache upload, and recorder-cache cleanup health without loopback playback.
...
342. ✅ Add Debian ALSA loopback short/long and X32 hardware short/long full-agent job capture/upload smokes.
