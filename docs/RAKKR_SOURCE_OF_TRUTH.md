...
344. ✅ Add fake-controller smoke coverage for agent node-config, node-heartbeat, recording-job capture start/runtime/too-small, recorder-cache cleanup/tracking sync and tracking failure, claim-next/status-poll/control-plane/channel-map failure/recovery health, controller-terminal handoff, and deterministic agent meter-health fault scores.
## Open Questions
