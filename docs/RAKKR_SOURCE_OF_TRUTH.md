...
| Recorder agent | 🟨 | Inventory, S16/S32 meters, controller capacity polling, bounded concurrent jobs, capture start/runtime/too-small/growth guards with structured stall evidence, profile rendering, controller-terminal job handoff, recorder-cache cleanup/delete-failure/tracking sync and tracking failure, clipping/flatline/low-signal/channel-correlation/heartbeat/node-config/claim-next/status-poll/control-plane/channel-map health, concurrent-safe health log, hardware-derived health-event controller sync |
...
| Health watchdog | 🟨 | Checked scheduled and agent low-signal, speech/noise, SNR, intelligibility, hum/static/broadband/correlation telemetry, deterministic agent meter-health quality evidence/fault scores, policy-tuned broadband quality alerts, synthetic calibration, field calibration helper, offline, local-log, metrics, timeline, central health workbench, node health lifecycle controls, audio-backend unavailable/recovery details, node heartbeat/config sync failure/recovery, recording-job capture start/runtime/too-small/stall evidence, recorder-cache cleanup/delete-failure/tracking sync, claim-next/status-poll/control-plane/channel-map failure and controller-terminal handoff, X32 hardware low-signal local/synced health, and PCH hardware flatline local/synced health; long-duration real-room validation remains |
...
- Recorder-cache delete-after-upload policies are pinned to jobs and executed by the agent after successful controller attach with delete-failure reporting.
...
- Debian test rig `/tmp/rakkr-recorder-agent` was refreshed from current source, and loopback meter JSON now includes speech/noise/broadband/static quality scores for the fixture smoke.
- Debian test rig fixture replay now verifies healthy delayed-stereo speech avoids alert-level channel correlation, low-volume speech drops below the low-signal threshold, duplicated channels produce alert-level correlation, and the daemon health log records clipping, low-signal, and channel-correlation events for those faults.
...
- Agent capture jobs fail and log health events for start/runtime failures, too-small/stalled output, render failures, cache upload failures, and terminal recording state.
...
- Agent system-health coverage exercises disk-pressure plus audio-backend unavailable/recovery detail transitions, and fake-controller smoke exercises stalled-capture structured growth evidence and render-failure health without audio hardware.
...
- `mise run check` includes fake-controller agent smoke coverage for job claim-next, heartbeat/status polling, capture start/runtime/too-small failure, claim-next/status-poll/control-plane/channel-map failure health, controller capacity override, bounded concurrent jobs, concurrent-safe local health log output, rendered MP3/VBR, recorder-cache delete-after-upload/tracking sync, tracking failure/max-bytes idle sweep, cache-upload failure handling, and controller stop requests without audio hardware, plus Rust helper coverage for delete failures.
...
- `mise run check` includes fake-controller agent smoke coverage for agent heartbeat, monitor chunk, and node-config sync failure/recovery without audio hardware.
...
328. ✅ Audit auth management read routes.
329. ✅ Audit node meter read route.
...
339. ✅ Deploy current recorder agent to Debian rig and validate fixture quality fields.
340. ✅ Extend Debian ALSA loopback fixture smoke to low-volume and duplicated-channel faults.
341. ✅ Add agent low-signal health and loopback daemon fault-event assertions.
...
344. ✅ Add fake-controller smoke coverage for agent node-config, node-heartbeat, recording-job capture start/runtime/too-small/stall evidence, recorder-cache cleanup/tracking sync and tracking failure, claim-next/status-poll/control-plane/channel-map failure/recovery health, controller-terminal handoff, deterministic agent meter-health fault scores, and recorder-cache delete-failure helper coverage.
## Open Questions
