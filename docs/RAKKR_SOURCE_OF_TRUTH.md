...
| Recorder agent | 🟨 | Inventory, S16/S32 meters, controller capacity polling, bounded concurrent jobs, capture start/runtime/too-small/growth guards, profile rendering, controller-terminal job handoff, recorder-cache cleanup/tracking sync and tracking failure, clipping/flatline/low-signal/channel-correlation/heartbeat/node-config/claim-next/status-poll/control-plane/channel-map health, concurrent-safe health log, hardware-derived health-event controller sync |
...
| Health watchdog | 🟨 | Checked scheduled and agent low-signal, speech/noise, SNR, intelligibility, hum/static/broadband/correlation telemetry, policy-tuned broadband quality alerts, synthetic calibration, field calibration helper, offline, local-log, metrics, timeline, central health workbench, node health lifecycle controls, audio-backend unavailable/recovery details, node heartbeat/config sync failure/recovery, recording-job capture start/runtime/too-small, recorder-cache cleanup/tracking sync, claim-next/status-poll/control-plane/channel-map failure and controller-terminal handoff, X32 hardware low-signal local/synced health, and PCH hardware flatline local/synced health; long-duration real-room validation remains |
...
- `mise run check` includes fake-controller agent smoke coverage for job claim-next, heartbeat/status polling, capture start/runtime/too-small failure, claim-next/status-poll/control-plane/channel-map failure health, controller capacity override, bounded concurrent jobs, concurrent-safe local health log output, rendered MP3/VBR, recorder-cache delete-after-upload/tracking sync, tracking failure/max-bytes idle sweep, cache-upload failure handling, and controller stop requests without audio hardware.
...
344. ✅ Add fake-controller smoke coverage for agent node-config, node-heartbeat, recording-job capture start/runtime/too-small, recorder-cache cleanup/tracking sync and tracking failure, claim-next/status-poll/control-plane/channel-map failure/recovery health, and controller-terminal handoff.
## Open Questions
