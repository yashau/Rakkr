...
| Recorder agent | 🟨 | Inventory, S16/S32 meters, controller capacity polling, bounded concurrent jobs, capture growth guards, profile rendering, clipping/flatline/low-signal/channel-correlation/heartbeat/node-config/status-poll health, concurrent-safe health log, hardware-derived health-event controller sync |
...
| Health watchdog | 🟨 | Checked scheduled and agent low-signal, speech/noise, SNR, intelligibility, hum/static/broadband/correlation telemetry, policy-tuned broadband quality alerts, synthetic calibration, field calibration helper, offline, local-log, metrics, timeline, central health workbench, node health lifecycle controls, node heartbeat/config sync failure/recovery, recording-job status-poll failure, X32 hardware low-signal local/synced health, and PCH hardware flatline local/synced health; long-duration real-room validation remains |
...
344. ✅ Add fake-controller smoke coverage for agent node-config, node-heartbeat, and recording-job status-poll failure/recovery health.
## Open Questions
