...
| Health watchdog      | 🟦 Scaffold  | Health event model/API exists; core non-AI checks first, AI quality analysis later |
...
Current scaffold status: health events have a shared type, Drizzle/Postgres table, and RBAC-gated `GET /api/v1/health-events` reader with filters for node, recording, schedule, severity, and result limit. The route is guarded by `health:read` and post-filters returned events through the same resource-scope model used by nodes, schedules, recordings, meters, and jobs, so a room or recorder denial also hides its health events. The Schedules execution detail page consumes this API to show linked health events next to recordings, jobs, and audit history. Event generation, acknowledge/suppress/resolve lifecycle actions, local node log sync, and DSP watchdog writers are still pending.
...
1. Add health-event writers, acknowledge/suppress/resolve lifecycle actions, and watchdog-backed recording quality markers.
