...
| Health watchdog | 🟨 | Checked scheduled and agent low-signal, speech/noise, SNR, intelligibility, hum/static/broadband/correlation telemetry, policy-tuned broadband quality alerts, synthetic calibration, field calibration helper, offline, local-log, metrics, timeline, central health workbench, node health lifecycle controls, node heartbeat/config sync failure/recovery, recording-job claim-next/status-poll/control-plane failure, X32 hardware low-signal local/synced health, and PCH hardware flatline local/synced health; long-duration real-room validation remains |
...
344. ✅ Add fake-controller smoke coverage for agent node-config, node-heartbeat, and recording-job claim-next/status-poll/control-plane failure/recovery health.
## Open Questions
