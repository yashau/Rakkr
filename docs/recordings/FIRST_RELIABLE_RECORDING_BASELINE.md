...
- Fake-controller smoke coverage proves agent job polling, status-poll failure health, controller capacity override, bounded concurrent jobs, capture/render handoff, concurrent-safe local health logging, MP3 VBR output, cache upload, cache-upload failure, and controller stop handling without hardware.
...
| Agent render/cache/stop/status-poll failure/controller-capacity/concurrency smoke | `scripts/agent-fake-controller-smoke.mjs` |
