...
- ALSA is the default capture and meter backend; PipeWire has first-class command presets for capture jobs and idle metering.
...
- Runtime inventory reports detected PipeWire and JACK command availability alongside collected audio interface backends; PipeWire can be selected as a managed capture/meter backend, while JACK remains availability-only.
...
- Remaining gaps: Linux loopback smoke execution on a recorder node, physical X32 validation, broader physical-device validation, and a JACK adapter are not complete.
...
| PipeWire capture and meter command presets | `crates/recorder-agent/src/config.rs`, `crates/recorder-agent/src/capture.rs`, `crates/recorder-agent/src/meter_command.rs`, `crates/recorder-agent/src/channel_map.rs`, `crates/recorder-agent/src/node_config.rs` |
