...
- The inventory probe command and `/proc/asound/pcm` path can be pinned for constrained Linux hosts and deterministic recovery smokes.
...
| ALSA inventory parsing, configurable probe command and `/proc/asound/pcm` path, stream/hw-params metadata, sysfs serials | `crates/recorder-agent/src/inventory.rs`, `crates/recorder-agent/src/config.rs` |
