...
- Linux `snd-aloop` smoke tasks can validate WAV capture, agent meters, render/channel-map output, clean/clipping/low-volume/channel-correlation fixture replay plus daemon health-log events, and a full fake-controller agent job that captures, uploads, and cleans up a loopback WAV.
