...
and Nodes need `node:read`, Health needs `health:read`, Schedules need
`schedule:read`, Recordings and Jobs need `recording:read`, Settings needs
`settings:read`, Audit needs `audit:read`, Access needs `auth:manage`. The header
...
| **Schedule detail** | `/schedules/$id` | Upcoming windows, linked recordings/jobs, quality timelines, health events, audit timeline.                                                                         | `schedule:read` + granular reads                                                 |
...
| **Settings**        | `/settings`      | Recording profiles, watchdog policies (+ calibration), upload providers/policies, retention, the upload runner, and channel maps (templates, assignments, rollout). | `settings:read`; mutations need `settings:manage`                                |
