...
test("recording routes deny users without required permissions", async () => {
  ...
  const responses = await Promise.all([
    ...
    requestJson(app, "/api/v1/recordings/export", "POST", {
      recordingIds: [recording().id],
    }),
    app.request("/api/v1/recordings/facets"),
    app.request(`/api/v1/recordings/${recording().id}`),
    app.request(`/api/v1/recordings/${recording().id}/context`),
    app.request(`/api/v1/recordings/${recording().id}/actions`),
    ...
    app.request("/api/v1/recording-jobs/job_missing/actions"),
    app.request("/api/v1/recording-jobs/job_missing/retry", { method: "POST" }),
    app.request("/api/v1/upload-queue"),
    app.request("/api/v1/upload-queue/upload_missing"),
    app.request("/api/v1/upload-queue/upload_missing/actions"),
  ...
  assert.deepEqual(
    Object.fromEntries(deniedEvents.map((event) => [event.action, event.permission]).sort()),
    {
      ...
      "recording_jobs.actions.read": "recording:read",
      ...
      "recordings.context.read": "recording:read",
      "recordings.actions.read": "recording:read",
      ...
      "recordings.upload_queue.actions.read": "recording:read",
      "recordings.upload_queue.detail.read": "recording:read",
