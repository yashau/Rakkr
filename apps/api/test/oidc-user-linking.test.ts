...
import { createDatabase, eq, users, type AzureAdOidcClaims } from "@rakkr/db";

const { LocalAuthService } = await import("../src/auth-service.js");
const { normalizeAzureAdOidcUser } = await import("../src/oidc-sync.js");

// Persistence-level linking runs only with a Postgres test DB (repo convention);
// in DB mode run with `--test-force-exit` (the client pool has no exposed close).
...
test(
  "DB: an OIDC login must NOT take over an existing local account by email (R38-OIDC-EMAIL-LINK)",
...
test(
  "DB: subject-linking creates one account and re-links it across email changes",
  { skip: dbUrl ? false : "requires RAKKR_API_TEST_DATABASE_URL" },
  async () => {
    const db = createDatabase(dbUrl!);
    const auth = new LocalAuthService(dbUrl);
...
test(
  "DB: a legacy email-linked OIDC row (no external_id) is adopted on next login, not duplicated",
  { skip: dbUrl ? false : "requires RAKKR_API_TEST_DATABASE_URL" },
  async () => {
    const db = createDatabase(dbUrl!);
    const auth = new LocalAuthService(dbUrl);
