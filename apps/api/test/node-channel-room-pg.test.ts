...
// persistence (audio_channels.room_id) was never exercised. Runs against an
// in-process PGlite (WASM Postgres) database, so it needs no running server.
// DATABASE_URL must be set BEFORE importing the stores.
const pglite = await createPgliteDatabase("node-channel-room-pg");
...
test("PostgresNodeStore.assignChannelRooms persists per-channel room assignments", async () => {
