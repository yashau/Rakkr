...
test("schedules render a data table with server-side pagination", async ({ page, request }) => {
  await loginAndOpen(page, request, "/schedules");
  ...
  await page.screenshot({ path: `${screenshotDir}/schedules-data-table.png` });
});

test("schedules page size selector drives the limit query param", async ({ page, request }) => {
  await loginAndOpen(page, request, "/schedules");
  await expect(page.getByRole("table")).toBeVisible();

  const [request_] = await Promise.all([
    page.waitForRequest(/\/api\/v1\/schedules\?.*limit=25/),
    page
