...
test("recording jobs render a data table with server-side pagination", async ({
  page,
  request,
}) => {
  ...
  await page.screenshot({ path: `${screenshotDir}/jobs-data-table.png` });
});

test("recording jobs page size selector drives the limit query param", async ({
  page,
  request,
}) => {
  await loginAndOpen(page, request, "/jobs");
  ...
  const [request_] = await Promise.all([
    page.waitForRequest(/\/api\/v1\/recording-jobs\?.*limit=25/),
    page
