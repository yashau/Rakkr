...
pub(crate) async fn reconcile_previous_recording_job(
    ...
    // Chunked recordings own N independently-uploaded chunks; a gapless pipe cannot
    // be transparently restarted, so recovery only retries locally-persisted but
    // unuploaded chunks then completes (or fails only if zero chunks were uploaded).
    if is_chunked_job_state(&state) {
        return reconcile_previous_chunked_recording_job(config, token, state).await;
    }
...
fn is_chunked_job_state(state: &AgentJobState) -> bool {
...
/// a chunk-file upload).
fn recovered_chunk_total(state: &AgentJobState) -> Option<u32> {
...
async fn reconcile_previous_chunked_recording_job(
    config: &AgentConfig,
    token: &str,
    state: AgentJobState,
) -> anyhow::Result<()> {
        ...
        let state =
            |chunk_total: Option<u32>, uploaded: Option<u32>, pending: Vec<u32>| AgentJobState {
                ...
                pending_chunks: pending
                    .into_iter()
                    .map(|index| crate::state::AgentPendingChunk {
                        index,
